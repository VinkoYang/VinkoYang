...
- content: 新增 research 条目《Path Planning Using Industrial Robotics for Automation of Route-Based Vibration Data Collection》（`_data/web/research.yml`，Avinash 主导，TurtleBot 4 / ROS 2 路线式振动巡检），含 YouTube 演示链接与配图 `images/research/2026_RouteVibrationRobot.png`；该条目经由 `_pages/videos.md` 的 links.video 自动渲染，同步出现在 /videos/。News 加一条对应消息。
