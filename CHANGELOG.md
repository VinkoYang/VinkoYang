...
- `assets/ref.bib`：新增 AHFE 2026 那篇合作论文 —— `@inproceedings{rahman2026human}`（A Human-Centered AI Task Management System for Cognitive Load Reduction and Decision Support in Industrial Plant Management，第六作者，DOI `10.54941/AHFE-Paper-0637`，PDF 已在 `papers/AHFE-Paper-0637.pdf`），以及对应的 `@incollection{talk2026ahfe_taskmanager}`（`keywords={talk}`，`data={2026-12-01}`，AHFE Hawaii International Conference，Dec 1–3 2026）。`volume` 暂缺：拿到的是预印版，刊头还印着 "Vol. XXX"，正式出版后再补。注意 PDF 的 `/Title` 元数据是 AHFE InDesign 模板的残留（"Effects of Swarm Size Variability on Operator Workload", Hunt et al.），不是这篇，标题作者都取自正文。
